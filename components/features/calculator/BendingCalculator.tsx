            ...
            <div className="lg:col-span-5 space-y-4">
                <div className="bg-[#0f172a] border border-white/10 rounded-xl p-4 shadow-xl">
                    <h3 className="text-white font-bold text-xs uppercase tracking-widest flex items-center gap-2 mb-4 border-b border-white/5 pb-2">
                        <Settings size={14} className="text-brand-blue-light" /> Parâmetros de Dobra
                    </h3>
                    <div className="grid grid-cols-3 gap-3">
                        <div><label className={labelClass}>Espessura (T)</label><input type="number" value={thickness} onChange={e => setThickness(e.target.value)} className={inputClass} /></div>
                        <div><label className={labelClass}>Raio Int. (R)</label><input type="number" value={radius} onChange={e => setRadius(e.target.value)} className={inputClass} /></div>
                        <div><label className={labelClass}>Ângulo (°)</label><input type="number" value={angle} onChange={e => setAngle(e.target.value)} className={inputClass} /></div>
                        <div><label className={labelClass}>Fator K</label><input type="number" step="0.01" value={kFactor} onChange={e => setKFactor(e.target.value)} className={inputClass} /></div>
                        <div><label className={labelClass}>Aba 1</label><input type="number" value={leg1} onChange={e => setLeg1(e.target.value)} className={inputClass} /></div>
                        <div><label className={labelClass}>Aba 2</label><input type="number" value={leg2} onChange={e => setLeg2(e.target.value)} className={inputClass} /></div>
                    </div>
                </div>
            </div>
            ...
            <div className="lg:col-span-7 space-y-4">
                <div className="bg-brand-blue-dark rounded-xl p-4 border border-brand-blue-light/20 shadow-lg relative overflow-hidden">
                    ...
                    <h3 className="text-white font-bold text-xs uppercase tracking-widest flex items-center gap-2 mb-2 z-10">
                        <FoldVertical size={14} className="text-brand-orange" /> Tamanho do Blank (Corte)
                    </h3>
                    ...
                    <div className="grid grid-cols-2 gap-3 border-t border-white/10 pt-3">
                        <div className="space-y-0.5">
                            <span className="text-[9px] text-gray-400 uppercase font-bold">Dedução (BD)</span>
                            <span className="text-lg font-mono font-bold text-red-400">-{results.bendDeduction.toFixed(2)} mm</span>
                        </div>
                        <div className="space-y-0.5">
                            <span className="text-[9px] text-gray-400 uppercase font-bold">Tolerância (BA)</span>
                ...
                <div className="bg-[#0f172a] border border-white/10 rounded-xl p-4 shadow-inner">
                    <div className="flex items-center gap-2 mb-2 border-b border-white/5 pb-1">
                        ...
                        <h4 className="text-xs font-bold text-white uppercase tracking-widest">Teoria</h4>
                    </div>
